...
class Network: ObservableObject {
    ...
    @Published var starredRepos: [StarredRepo] = []
    ...
    func getRepos() {
        guard let url = URL(string: "http://localhost:5002/api/repos") else {
    ...
    func getWeeklyCommits(repoName: String) {
        guard let url = URL(string: "http://localhost:5002/api/commits/dayly") else {
            fatalError("missing url")
        }
        let token = UserDefaults.standard.string(forKey: "access_token")
        print(token!)
    ...
    func getAllCommits(repoName: String, index: Int) {
        guard let url = URL(string: "http://localhost:5002/api/commits/weekly") else {
            fatalError("missing url")
        }
        let token = UserDefaults.standard.string(forKey: "access_token")
        print(token!)
    ...
    func getCommitDetails(repoName: String) {
        guard let url = URL(string: "http://localhost:5002/api/commits/stats") else {
            fatalError("missing url")
        }
        let token = UserDefaults.standard.string(forKey: "access_token")
        let json: [String: Any] = ["access_token": "\(token!)", "repoName": "\(repoName)"]
    ...
    func getQuery(languages: [Languages]) {
        
    }
    func getStarredRepos() {
        guard let url = URL(string: "http://localhost:5002/api/repos/starred") else {
            fatalError("missing url")
        }
        let token = UserDefaults.standard.string(forKey: "access_token")
        let json: [String: Any] = ["access_token": "\(token!)"]
        ...
        let dataTask = URLSession.shared.dataTask(with: urlRequest) { data, response, error in
            ...
            if response.statusCode == 200 {
                ...
                DispatchQueue.main.async {
                    do {
                        let decodedRepos = try JSONDecoder().decode([StarredRepo].self, from: data)
                        self.starredRepos = decodedRepos
                        print("starred repos are \(decodedRepos)")
    ...
    func getLanguagesPerRepo(repoName: String) {
        guard let url = URL(string: "http://localhost:5002/api/repos/language") else {
            fatalError("missing url")
        }
        let token = UserDefaults.standard.string(forKey: "access_token")
        print(token!)
    ...
    func getTotalLines(repos: [Repo]) {
        guard let url = URL(string: "http://localhost:5002/api/total") else {
            fatalError("missing url")
        }
        let token = UserDefaults.standard.string(forKey: "access_token")
        print(token!)
